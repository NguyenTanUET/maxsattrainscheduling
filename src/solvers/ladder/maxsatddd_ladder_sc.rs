...
pub struct MaxSatDddLadderScSettings {
    ...
    /// "use_sc_fixed_precedence" but this is NOT the Sequential Counter
    /// for Ladder (SC) AMO encoding — see [`add_sc_amo`] for that.
    ...
    /// Within the interval-graph clique-cover encoding, use the SC
    ...
    /// Seed fixed precedence rows from earliest time points (only used when SC is enabled).
...
// ─── Tunables for SC AMO experimentation ───────────────────────────────────
//
...
// `maxsatddd_ladder_sc` and `incremental_sat` use byte-identical
...
fn add_fixed_precedence_row<L: satcoder::Lit>(
    ...
    if use_eager_chain_expansion {
        ...
        // [`add_sc_amo`]. The legacy field name `use_sc_fixed_precedence`
        // referred to this expansion heuristic, not the paper's SC; it has
        // been renamed to `use_eager_chain_expansion` to avoid confusion.
